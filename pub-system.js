	...
	'flickity':function(){//手机滑动
		if($(".ff-gallery").length){
			...
			$("<link>").attr({
				rel: "stylesheet",
				type: "text/css",
				href: "https://cdn.staticfile.org/flickity/2.1.1/flickity.min.css"
			}).appendTo("head");
			$.getScript("https://cdn.staticfile.org/flickity/2.1.1/flickity.pkgd.min.js", function(){
...
'image': {//图片
	'lazyload': function(){//延迟加载
		...
		$.getScript("https://cdn.bootcss.com/jquery_lazyload/1.9.7/jquery.lazyload.js", function(response, status) {
			$("img.ff-img").lazyload({
				placeholder : "http://i1.fuimg.com/692418/c6e7060862a212c7.png",
				effect : "fadeIn",
				failurelimit: 15
				//threshold : 400
...
'search': {//搜索
	...
	'autocomplete': function(){//3.5增加搜索结果控制 4.1update
		...
		if( $limit > 0){
			...
			$.getScript("https://cdn.staticfile.org/jquery.devbridge-autocomplete/1.4.9/jquery.autocomplete.min.js", function(response, status) {
				$('.ff-wd').autocomplete({
